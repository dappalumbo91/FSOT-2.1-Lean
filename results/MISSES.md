...
| BSD-RANK | — | \(\mathrm{ord}\,L\) for rank \(\ge 1\) | Rank 0 is \(L(1)\neq 0\), not \(L\) magnitude. 17a1 raw \(L(1)\) mis-fires as rank 3; \(\mathrm{Sha}_{\mathrm{an}}=1\). 19a1 oos. First-of-rank 0..4 still labels the first curves. | Did not nearest-template arbitrary \(L(1)\). Did not \(\pi^2\cdot\mathrm{POOF}\). |
