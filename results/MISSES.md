...
| HODGE-CLASS | — | Extra Hodge classes beyond \(C_{24}\) | \(C_8\) \([\mathrm{plane}]\). \(C_{12}\) \([\mathrm{cubic\ scroll}]\). \(C_{18}\) \([\mathrm{elliptic\ ruled}]\). \(C_{20}\) \([\mathrm{Veronese}]\). \(C_{24}\) \([\mathrm{nodal\ sextic\ del\ Pezzo}]\) (\(d=L_2(L_6+2)-(2L_2)^2=24\)). No associated K3. Remainder: \(C_{30}\), \(C_{32}\), later extra discriminants, general 4-folds. | Did not steal 25−1 for \(\chi(\mathrm{K3})=24\). Did not pad \(d=24\) as \(8\cdot 3\). |
