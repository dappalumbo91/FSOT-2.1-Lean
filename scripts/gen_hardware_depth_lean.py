#!/usr/bin/env python3
"""Generate Lean priors for cache / interconnect / C-parity / depth spine."""
...
from hardware_depth_bridge_lib import BUILDERS, LEAN_MAP, output_path  # noqa: E402
...
def main() -> int:
    ...
    for domain in args.only or sorted(BUILDERS.keys()):
        ...
        if not bench_path.is_file():
            print(f"Skip {domain}: run build_hardware_depth_bridge.py first")
            continue
        bench = json.loads(bench_path.read_text(encoding="utf-8"))
        prefix, lean_domain, sign_th, module_stem = LEAN_MAP[domain]
        text = extension_priors_lean(
            module_title=f"FSOT Formal {module_stem} — hardware depth ({domain}).",
            ...
            med=float(
                bench.get("pooled_median_error_pct") or bench.get("median_error_pct") or 0.0
            ),
            d_eff=int(bench.get("D_eff", 11)),
            gate_pct=0.5,
        )
