#!/usr/bin/env python3
"""Generate Lean priors for Tier 96 circuit component emergence panels."""
...
from circuit_component_emergence_lib import BUILDERS, LEAN_MAP, output_path  # noqa: E402
...
def main() -> int:
    ...
    for domain in args.only or sorted(BUILDERS.keys()):
        ...
        if not bench_path.is_file():
            print(f"Skip {domain}: run build_circuit_component_emergence_benchmarks.py first")
            continue
        bench = json.loads(bench_path.read_text(encoding="utf-8"))
        prefix, lean_domain, sign_th, module_stem = LEAN_MAP[domain]
        text = extension_priors_lean(
            module_title=f"FSOT Formal {module_stem} — Tier 96 circuit emergence ({domain}).",
