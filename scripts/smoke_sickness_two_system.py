...
def main() -> int:
    h_c, h_e = fsot_scaled(HOST["measured_bp"], HOST["domain"])
    p_c, p_e = fsot_scaled(PATHOGEN["measured_bp"], PATHOGEN["domain"])
    s_h = domain_scalar("Biology")
    s_p = domain_scalar("Biochemistry")
    ...
    payload = {
        ...
        "host": {**HOST, "computed_bp": h_c, "error_pct": h_e, "S": s_h},
        "pathogen": {**PATHOGEN, "computed_bp": p_c, "error_pct": p_e, "S": s_p},
    ...
    print(
        f"host {h_e:.4f}%  pathogen {p_e:.4f}%  kappa={kap:.6f}  "
        f"tau_h={payload['process_time_days_host_d12']:.3f}d "
        f"tau_p={payload['process_time_days_path_d13']:.3f}d"
    )
    if h_e > 0.5 or p_e > 0.5:
