#!/usr/bin/env python3
"""Refine §51 aqueous logS tail outliers (toluene, aniline, paracetamol)."""
...
SECTION = "§51 Solubility logS"

# Expanded seed search: benzene uses -PHI; toluene adds -PSI coupling.
LOGS_OVERRIDES: dict[str, dict[str, object]] = {
    "toluene": {
...
def _patch_smiles_dataset(path: Path) -> int:
    ...
    for row in doc.get("records") or []:
        ...
        override = LOGS_OVERRIDES.get(name)
        if not override:
            continue
...
def _refresh_registry(dataset_path: Path) -> None:
    ...
    registry["smiles_lab"]["section51_logS_patched_at"] = datetime.now(timezone.utc).isoformat()
...
def main() -> int:
    ...
    print(f"Patched {n} §51 records in {args.dataset}")

    if args.unified_db.exists():
        u = _patch_unified_db(args.unified_db)
        print(f"Patched {u} unified-db §51 rows in {args.unified_db}")

    if args.also_desktop and DESKTOP_DATASET.exists():
        d = _patch_smiles_dataset(DESKTOP_DATASET)
        print(f"Patched {d} §51 records in {DESKTOP_DATASET}")

    if n != len(LOGS_OVERRIDES):
        print(f"Expected {len(LOGS_OVERRIDES)} updates, got {n}", file=sys.stderr)
        return 1
