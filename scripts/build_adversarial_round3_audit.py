...
def build() -> dict:
    ...
    gaps: list[dict] = [
        _gap(
            ...
            closed=explicit_link_hit >= 99.0 and conj_pct >= 50.0,
            evidence="data/structural_bundle_ledger.json",
        ),
        _gap(
            "oracle_replay_not_independent_proof",
            "documented_debt",
            f"{oracle_n} Bounds/priors rows are oracle replay (sampling_oracle, witness_instantiation, certified_interval) — replayed not proved in Mathlib depth.",
            "oracle_debt_ledger.json inventories triangulation_class; excluded from atomic triangulation headline.",
            closed=False,
        ...
        _gap(
            ...
            "info",
            f"{tier_fails} benchmark domains fail tier_scalar_pass (max record ≤0.05%) while green_gate uses pooled median ≤0.5%.",
            "benchmark_margin_audit.json tier_scalar_fail_count — aspirational precision tier, not github_ready blocker.",
            closed=False,
