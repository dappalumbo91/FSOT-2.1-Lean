#!/usr/bin/env python3
"""Generate Lean priors for FSOT-GPU CUDA / processor / RAM residual panels.

Hooks into the same multiprover path as other extension domains:
  benchmarks → *Priors.lean → margin audit → scientific_catalog_spine → Coq/Isabelle.
"""
...
from fsot_gpu_cuda_bridge_lib import BUILDERS, LEAN_MAP, output_path  # noqa: E402
...
def main() -> int:
    ...
    for domain in args.only or sorted(BUILDERS.keys()):
        ...
        if not bench_path.is_file():
            print(f"Skip {domain}: run build_fsot_gpu_cuda_bridge.py first")
            continue
        bench = json.loads(bench_path.read_text(encoding="utf-8"))
        prefix, lean_domain, sign_th, module_stem = LEAN_MAP[domain]
        text = extension_priors_lean(
            module_title=(
                f"FSOT Formal {module_stem} — GPU/CUDA/processor/RAM "
                f"seed residual panel ({domain})."
            ),
