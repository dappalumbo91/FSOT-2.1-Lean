#!/usr/bin/env python3
"""Generate Lean priors for Tier 59–60 domains."""
...
DOMAIN_CONFIG: dict[str, tuple[str, str, int, str]] = {
    "Material_Property_Verification_Scaffold": (
        "material_property_verification_scaffold",
        "MaterialPropertyVerificationScaffoldPriors",
        15,
        "material_property_verification_scaffold_benchmark.json",
    ),
    "Fuel_Thermochemistry_Public_Anchors": (
        "fuel_thermochemistry_public_anchors",
        "FuelThermochemistryPublicAnchorsPriors",
        16,
        "fuel_thermochemistry_public_anchors_benchmark.json",
    ),
    "SIMBAD_Stellar_Identity_Deep": (
        "simbad_stellar_identity_deep",
        "SIMBADStellarIdentityDeepPriors",
        20,
        "simbad_stellar_identity_deep_benchmark.json",
    ),
    "Gaia_Astrometry_Panel_Deep": (
        "gaia_astrometry_panel_deep",
        "GaiaAstrometryPanelDeepPriors",
        20,
        "gaia_astrometry_panel_deep_benchmark.json",
    ),
}
...
def build_lean(bench: dict, prefix: str, module_stem: str, d_eff: int) -> str:
    ...
    return f"""/-
  FSOT Formal {module_stem} — Tier 59/60 public material/fuel scaffold + live astrometry.
