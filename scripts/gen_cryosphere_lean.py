#!/usr/bin/env python3
"""Generate FSOT/Formal/CryospherePriors.lean."""
...
BENCH = ROOT / "data" / "cryosphere_benchmark.json"
OUTPUT = ROOT / "FSOT" / "Formal" / "CryospherePriors.lean"
...
def cryosphere_month_count : ℕ := {n}
def cryosphere_station_count : ℕ := {stn}
def cryosphere_match_count : ℕ := {match_n}
def cryosphere_D_eff : ℕ := {d_eff}
def cryosphere_match_rate : ℝ := ({rate} : ℝ)
...
theorem cryosphere_bundle :
    cryosphere_month_count = {n} ∧
    cryosphere_station_count = {stn} ∧
    cryosphere_match_count = {match_n} ∧
    cryosphere_D_eff = {d_eff} ∧
