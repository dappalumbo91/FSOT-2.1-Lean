# Field products (hub entry points)

**Live pin:** AEB2AD. These are **application CLIs** on public tables. They quote the oracle; they do not fork `vendor/fsot_compute.py`.

Ledger: **B corrects** (catalog residual). Not Ledger A predict. Not a Clay prize. Not ECMWF beaten.

| Product | Command | Kill |
|---------|---------|------|
| Chemistry lab | `python products/chemistry_lab.py` | Fitted Trouton; \(n_D\) as MW |
| Weather windows | `python products/weather_monitor.py` | S2S week-3 beat claim |
| Seismology windows | `python products/seismo_windows.py` | UTC hypocenter as 0.5% |
| MPCORB catalog | `python products/catalog_mpcorb.py` | One \(\varepsilon\) per asteroid |
| Optics \(n_D\) | `python products/optics_nd.py` | Static water \(\varepsilon\sim 80\) as \(n^2\) |

Always-on siblings (Genetics, Quantum, Neural, GPU, Materials) stay out of this folder.
