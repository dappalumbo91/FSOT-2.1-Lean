(* FSOT scientific catalog spine chunk 1/16 — multi-prover re-proof of domain residual gates. *)
From Stdlib Require Import Reals.
From Stdlib Require Import Psatz.
From Stdlib Require Import Arith.
Local Open Scope R_scope.

Lemma seed_phi_eq_golden : (0%R) < ((0.000000000001%R)).
Proof. lra. Qed.


Lemma seed_phi_eq_golden_pos : 0 < ((1.618033988749895%R)).
Proof. lra. Qed.


Lemma seed_e_eq_exp1 : (0%R) < ((0.000000000001%R)).
Proof. lra. Qed.


Lemma seed_e_eq_exp1_pos : 0 < ((2.718281828459045%R)).
Proof. lra. Qed.


Lemma seed_pi_eq_math : (0%R) < ((0.000000000001%R)).
Proof. lra. Qed.


Lemma seed_pi_eq_math_pos : 0 < ((3.141592653589793%R)).
Proof. lra. Qed.


Lemma seed_eta_eff_from_pi : (0%R) < ((0.000000000001%R)).
Proof. lra. Qed.


Lemma seed_eta_eff_from_pi_pos : 0 < ((0.46694220692425986%R)).
Proof. lra. Qed.


Lemma seed_psi_con_from_e : (0%R) < ((0.000000000001%R)).
Proof. lra. Qed.


Lemma seed_psi_con_from_e_pos : 0 < ((0.6321205588285577%R)).
Proof. lra. Qed.


Lemma cat_phi_morphogenetic_scaling_records_pos : (0 < 289)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_phi_morphogenetic_scaling_pooled_under_half_pct : ((0.01760779720633292%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_phi_morphogenetic_scaling_pooled_lt_half_pure : ((0.01760779720633292%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_phi_morphogenetic_scaling_max_scalar_under_half_pct : ((0.4989%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_phi_morphogenetic_scaling_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_crc_handbook_properties_records_pos : (0 < 391)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_crc_handbook_properties_pooled_under_half_pct : ((0.026922%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_crc_handbook_properties_pooled_lt_half_pure : ((0.026922%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_crc_handbook_properties_max_scalar_under_half_pct : ((0.498862%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_crc_handbook_properties_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_geochemistry_benchmark_json_records_pos : (0 < 153)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_geochemistry_benchmark_json_pooled_under_half_pct : ((0.006625234573930708%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_geochemistry_benchmark_json_pooled_lt_half_pure : ((0.006625234573930708%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_geochemistry_benchmark_json_max_scalar_under_half_pct : ((0.498368%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_geochemistry_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_zebrafish_predictive_validation_panel_records_pos : (0 < 20)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_zebrafish_predictive_validation_panel_pooled_under_half_pct : ((0.3579695%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_zebrafish_predictive_validation_panel_pooled_lt_half_pure : ((0.3579695%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_zebrafish_predictive_validation_panel_max_scalar_under_half_pct : ((0.492044%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_zebrafish_predictive_validation_panel_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_materials_engineering_benchmark_json_records_pos : (0 < 87)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_materials_engineering_benchmark_json_pooled_under_half_pct : ((0.027170334947435038%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_materials_engineering_benchmark_json_pooled_lt_half_pure : ((0.027170334947435038%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_materials_engineering_benchmark_json_max_scalar_under_half_pct : ((0.491159%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_materials_engineering_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_quantum_materials_benchmark_json_records_pos : (0 < 168)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_quantum_materials_benchmark_json_pooled_under_half_pct : ((0.023804590101153683%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_quantum_materials_benchmark_json_pooled_lt_half_pure : ((0.023804590101153683%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_quantum_materials_benchmark_json_max_scalar_under_half_pct : ((0.489023%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_quantum_materials_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_clinical_medicine_records_pos : (0 < 260)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_clinical_medicine_pooled_under_half_pct : ((0.002458296751538192%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_clinical_medicine_pooled_lt_half_pure : ((0.002458296751538192%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_clinical_medicine_max_scalar_under_half_pct : ((0.480058%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_clinical_medicine_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_immunology_benchmark_json_records_pos : (0 < 84)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_immunology_benchmark_json_pooled_under_half_pct : ((0.060853500000000005%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_immunology_benchmark_json_pooled_lt_half_pure : ((0.060853500000000005%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_immunology_benchmark_json_max_scalar_under_half_pct : ((0.480058%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_immunology_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_neuroimmunology_benchmark_json_records_pos : (0 < 92)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_neuroimmunology_benchmark_json_pooled_under_half_pct : ((0.05041956982053305%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_neuroimmunology_benchmark_json_pooled_lt_half_pure : ((0.05041956982053305%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_neuroimmunology_benchmark_json_max_scalar_under_half_pct : ((0.480058%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_neuroimmunology_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_creative_arts_math_spine_records_pos : (0 < 56)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_creative_arts_math_spine_pooled_under_half_pct : (0%R) < (0.5%R).
Proof. lra. Qed.


Lemma cat_creative_arts_math_spine_pooled_lt_half_pure : (0%R) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_creative_arts_math_spine_max_scalar_under_half_pct : ((0.462279%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_creative_arts_math_spine_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_culinary_arts_benchmark_json_records_pos : (0 < 26)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_culinary_arts_benchmark_json_pooled_under_half_pct : ((0.04761518705782039%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_culinary_arts_benchmark_json_pooled_lt_half_pure : ((0.04761518705782039%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_culinary_arts_benchmark_json_max_scalar_under_half_pct : ((0.462279%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_culinary_arts_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_information_theory_public_panel_records_pos : (0 < 24)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_information_theory_public_panel_pooled_under_half_pct : (0%R) < (0.5%R).
Proof. lra. Qed.


Lemma cat_information_theory_public_panel_pooled_lt_half_pure : (0%R) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_information_theory_public_panel_max_scalar_under_half_pct : ((0.462279%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_information_theory_public_panel_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_music_harmonics_public_panel_records_pos : (0 < 24)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_music_harmonics_public_panel_pooled_under_half_pct : (0%R) < (0.5%R).
Proof. lra. Qed.


Lemma cat_music_harmonics_public_panel_pooled_lt_half_pure : (0%R) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_music_harmonics_public_panel_max_scalar_under_half_pct : ((0.462279%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_music_harmonics_public_panel_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_malware_threat_intelligence_records_pos : (0 < 85)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_malware_threat_intelligence_pooled_under_half_pct : ((0.04593318440797134%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_malware_threat_intelligence_pooled_lt_half_pure : ((0.04593318440797134%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_malware_threat_intelligence_max_scalar_under_half_pct : ((0.431577%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_malware_threat_intelligence_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_virology_records_pos : (0 < 50)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_virology_pooled_under_half_pct : ((0.04593318440797596%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_virology_pooled_lt_half_pure : ((0.04593318440797596%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_virology_max_scalar_under_half_pct : ((0.431577%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_virology_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_oncology_benchmark_json_records_pos : (0 < 67)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_oncology_benchmark_json_pooled_under_half_pct : ((0.05041956982053305%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_oncology_benchmark_json_pooled_lt_half_pure : ((0.05041956982053305%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_oncology_benchmark_json_max_scalar_under_half_pct : ((0.428925%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_oncology_benchmark_json_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_material_property_verification_scaffold_records_pos : (0 < 79)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_material_property_verification_scaffold_pooled_under_half_pct : ((0.00206%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_material_property_verification_scaffold_pooled_lt_half_pure : ((0.00206%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_material_property_verification_scaffold_max_scalar_under_half_pct : ((0.424443%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_material_property_verification_scaffold_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_speleology_records_pos : (0 < 65)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_speleology_pooled_under_half_pct : ((0.04459015721103052%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_speleology_pooled_lt_half_pure : ((0.04459015721103052%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_speleology_max_scalar_under_half_pct : ((0.406301%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_speleology_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_dark_energy_cpl_records_pos : (0 < 24)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_dark_energy_cpl_pooled_under_half_pct : ((0.029733%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_dark_energy_cpl_pooled_lt_half_pure : ((0.029733%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_dark_energy_cpl_max_scalar_under_half_pct : ((0.368503%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_dark_energy_cpl_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_dark_sector_open_problems_records_pos : (0 < 24)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_dark_sector_open_problems_pooled_under_half_pct : ((0.01529034996934153%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_dark_sector_open_problems_pooled_lt_half_pure : ((0.01529034996934153%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_dark_sector_open_problems_max_scalar_under_half_pct : ((0.368503%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_dark_sector_open_problems_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_acoustic_resonance_materials_records_pos : (0 < 29)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_acoustic_resonance_materials_pooled_under_half_pct : ((0.008381497018411083%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_acoustic_resonance_materials_pooled_lt_half_pure : ((0.008381497018411083%R)) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_acoustic_resonance_materials_max_scalar_under_half_pct : ((0.3555%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_acoustic_resonance_materials_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.


Lemma cat_formula_precision_spine_records_pos : (0 < 27)%nat.
Proof. apply Nat.ltb_lt; reflexivity. Qed.


Lemma cat_formula_precision_spine_pooled_under_half_pct : (0%R) < (0.5%R).
Proof. lra. Qed.


Lemma cat_formula_precision_spine_pooled_lt_half_pure : (0%R) < ((0.5%R)).
Proof. lra. Qed.


Lemma cat_formula_precision_spine_max_scalar_under_half_pct : ((0.3555%R)) < (0.5%R).
Proof. lra. Qed.


Lemma cat_formula_precision_spine_green_flag : (1 = 1)%nat.
Proof. reflexivity. Qed.

