...
class OpenTrack:
    ...
    last_t: int
    missing: int = 0
    last_phys: np.ndarray | None = None
...
class SequenceTracker:
    ...
    def _mitosis_links(self, parent: OpenTrack, child_indices: list[int],
        ...
        c_phys = phys_coords(children)
        use_vel = os.environ.get("FSOT_MITOSIS_VELOCITY", "1") == "1"
        vel = None
        if use_vel and parent.last_phys is not None:
            vel = p_phys - parent.last_phys
            if float(np.linalg.norm(vel)) < 0.5:
                vel = None
        ...
        for j in child_indices:
            ...
            if d_um > MITOSIS_DAUGHTER_MAX_UM:
                continue
            prob = link_edge_prob_refined(
                d_um, _vol_key(parent.cell), _vol_key(children[j]),
            )
            if prob < _edge_threshold():
                continue
            score = prob
            if vel is not None:
                new_vel = c_phys[j] - p_phys
                vn = float(np.linalg.norm(vel) * np.linalg.norm(new_vel))
                if vn > 1e-6:
                    align = max(0.0, float(np.dot(vel, new_vel) / vn))
                    score *= 0.55 + 0.45 * align
            cand.append((score, j))
    ...
    def advance(self, t: int, cells: list[dict], global_ids: list[int]) -> None:
        ...
        for track in sorted(self.open_tracks, key=lambda tr: tr.missing):
            if not available:
                break
            if track.last_phys is None and track.missing == 0:
                track.last_phys = phys_coords([track.cell])[0].copy()
            matched = self._mitosis_links(track, sorted(available), cells, global_ids)
            if matched:
                linked_parents.add(track.global_id)
            for j in matched:
                available.discard(j)
                parent_phys = phys_coords([track.cell])[0]
                next_open.append(OpenTrack(
                    global_ids[j], cells[j], t, 0,
                    last_phys=parent_phys.copy(),
                ))
        ...
        for gap in range(1, GAP_MAX_FRAMES + 1):
            ...
            for j in matched:
                parent_phys = None
                for tr in layer:
                    if tr.global_id in parents_hit:
                        parent_phys = phys_coords([tr.cell])[0]
                        break
                next_open.append(OpenTrack(
                    global_ids[j], cells[j], t, 0,
                    last_phys=parent_phys.copy() if parent_phys is not None else None,
                ))
