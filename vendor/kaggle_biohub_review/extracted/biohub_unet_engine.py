...
def _build_config(weights: Path) -> PredictConfig:
    return PredictConfig(
        det_threshold=float(os.environ.get("CELLMOT_DET_THRESHOLD", "0.99")),
...
def _load_engine(weights: Path | None = None, device: str | None = None):
    ...
    cfg = _build_config(weights)
    return model, cfg, window_size, downsample, dev, weights
...
def predict_dataset(
    ...
    model, cfg, window_size, downsample, device, _ = _load_engine(weights)
