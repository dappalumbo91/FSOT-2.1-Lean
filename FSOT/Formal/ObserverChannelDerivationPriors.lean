...
theorem obs_ch_pooled_median_under_five_pct :
    obs_ch_pooled_median_error_pct < (5 : ℝ) := by
  unfold obs_ch_pooled_median_error_pct; norm_num

theorem obs_ch_headline_median_under_five_pct :
    obs_ch_headline_median_error_pct < (5 : ℝ) := by
...
theorem obs_ch_bundle :
    ...
    obs_ch_pooled_median_error_pct < (5 : ℝ) ∧
    obs_ch_beats_sota_headlines > 0 := by
  refine ⟨?h1, ?h2, ?h3⟩
  · unfold obs_ch_observable_count; norm_num
  · exact obs_ch_pooled_median_under_five_pct
