...
theorem network_inet_pooled_median_under_five_pct :
    network_inet_pooled_median_error_pct < (5 : ℝ) := by
  unfold network_inet_pooled_median_error_pct; norm_num

theorem network_inet_headline_median_under_five_pct :
    network_inet_headline_median_error_pct < (5 : ℝ) := by
...
theorem network_inet_bundle :
    ...
    network_inet_pooled_median_error_pct < (5 : ℝ) ∧
    network_inet_beats_sota_headlines > 0 := by
  refine ⟨?h1, ?h2, ?h3⟩
  · unfold network_inet_observable_count; norm_num
  · exact network_inet_pooled_median_under_five_pct
