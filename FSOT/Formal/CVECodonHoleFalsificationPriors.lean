...
theorem cve_hole_pooled_median_under_half_pct :
    cve_hole_pooled_median_error_pct < (0.5 : ℝ) := by
  unfold cve_hole_pooled_median_error_pct; norm_num

theorem cve_hole_headline_median_under_half_pct :
    cve_hole_headline_median_error_pct < (0.5 : ℝ) := by
...
theorem cve_hole_bundle :
    ...
    cve_hole_pooled_median_error_pct < (0.5 : ℝ) ∧
    cve_hole_beats_sota_headlines > 0 := by
  refine ⟨?h1, ?h2, ?h3⟩
  · unfold cve_hole_observable_count; norm_num
  · exact cve_hole_pooled_median_under_half_pct
