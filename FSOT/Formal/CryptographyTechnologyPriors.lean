...
theorem crypto_tech_pooled_median_under_five_pct :
    crypto_tech_pooled_median_error_pct < (5 : ℝ) := by
  unfold crypto_tech_pooled_median_error_pct; norm_num

theorem crypto_tech_headline_median_under_five_pct :
    crypto_tech_headline_median_error_pct < (5 : ℝ) := by
...
theorem crypto_tech_bundle :
    ...
    crypto_tech_pooled_median_error_pct < (5 : ℝ) ∧
    crypto_tech_beats_sota_headlines > 0 := by
  refine ⟨?h1, ?h2, ?h3⟩
  · unfold crypto_tech_observable_count; norm_num
  · exact crypto_tech_pooled_median_under_five_pct
