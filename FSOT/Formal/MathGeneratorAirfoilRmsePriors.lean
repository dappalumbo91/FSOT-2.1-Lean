...
theorem math_generator_airfoil_rmse_bundle :
    ...
    math_generator_airfoil_rmse_D_eff = 5 ∧
    math_generator_airfoil_rmse_median_error_pct < (5 : ℝ) ∧
    raw_S (get_domain_params "particle") > 0 := by
  refine ⟨
    by unfold math_generator_airfoil_rmse_observable_count; decide,
    by unfold math_generator_airfoil_rmse_D_eff; decide,
