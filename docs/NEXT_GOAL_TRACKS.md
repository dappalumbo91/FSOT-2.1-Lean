...
Polyisoprene \(T_g\): \(\varphi^{10}+\varphi^9\) is \(\varphi^{11}=199.005\) K against the cited 200 K (Mark / CRC / Debenedetti). The shortfall is the live compactification factor \(C_{\mathrm{eff}}\), not a new coefficient and not the integer 1: \(\varphi^{11}+C_{\mathrm{eff}}\) at 0.019%. Tungsten atomization: \(e^4+e^7-\pi^5\) was 845.21 against NIST-JANAF 849.4 kJ/mol. The missing piece is \(\varphi^3\), already used on this table: \(e^4+e^7-\pi^5+\varphi^3\) at 0.0056%.
