...
| **Hodge conjecture** | Hodge classes on a projective complex manifold are algebraic cycles (rational) | Lefschetz; associated K3; \(C_8\) \([\mathrm{plane}]\) and \(C_{12}\) \([\mathrm{scroll}]\) extra classes algebraic. Remainder: later extra discriminants, general 4-folds. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
