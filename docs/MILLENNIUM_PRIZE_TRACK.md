...
| **Navier–Stokes existence and smoothness** | Global smooth (or blow-up) solutions of 3D incompressible NSE | 1D Stokes, \(\kappa\), 2D enstrophy, Kolmogorov \(4/5=1-1/D_{\mathrm{particle}}\). Existence on \(\mathbb{R}^3\) open. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
| **P versus NP** | Proof that P=NP or P≠NP | Grover \(1/2\) (QI); Cook–Levin SAT named. Not P=?NP. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
| **Riemann hypothesis** | All non-trivial zeros of \(\zeta\) have real part \(1/2\) | First-zero \(e/\gamma^3\); \(N(T)\) C-lock; \(\lvert S\rvert\le 1/e\); typical \(\lvert S\rvert=\mathrm{POOF}\). Sign unsolved. Not RH. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
| **Birch and Swinnerton-Dyer** | Rank of \(E(\mathbb{Q})\) equals order of vanishing of \(L(E,s)\) at \(s=1\) | Ranks 0–3: \(L=\sqrt{\varphi}/D_{\mathrm{particle}}\), \(L'=2\cdot\mathrm{POOF}\), \(\mathrm{Reg}=\mathrm{POOF}\), \(\mathrm{Reg}=e\cdot\mathrm{POOF}\). No map \(E\mapsto\) rank. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
| **Hodge conjecture** | Hodge classes on a projective complex manifold are algebraic cycles (rational) | Lefschetz (1,1); (2,2) on \(\mathbb{CP}^3\); hard Lefschetz. Primitive (2,2) on general \(X\) open. | Native **probe**. Clay **OPEN_NOT_CLAIMED** |
