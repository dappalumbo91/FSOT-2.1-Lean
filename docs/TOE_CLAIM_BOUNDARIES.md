...
## Machine flag: `undeniable_toe_claim` (depth audit)

Computed by `python scripts/build_verification_depth_audit.py` → `data/verification_depth_audit.json`.
...
| **`full_mathlib_rederivation_of_all_lemmas: false`** | Always honest — does **not** claim every intermediate Lean lemma is independently re-derived in Coq/Isabelle/F* |
...
**Forbidden reading of the flag:** “classical continuum YM path-integral mass gap proved,” “every Mathlib proof chain rebuilt in four provers,” or “peer review complete.”
