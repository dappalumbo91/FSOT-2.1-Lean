...
| A SOTA beat as 0.5% green | Riemann n=2..10 locked-\(C\) point-T (1.63%) is POOF-amplitude interacting bleed vs smooth counting — Odlyzko sits in the 1/e Gram band; typical \(\lvert S\rvert=\mathrm{POOF}\). Glueball \(\varphi^2+1\) vs Teper 1997 (0.876%) is σ-units. | Do not stuff the point residual. Do not Euler-invert. Sign of the push is still open. |
| Glueball 6% vs AT2020 \(M/\sqrt{\sigma}\) as an FSOT miss | 1997 3.65 and AT2020 3.405 **disagree by ~7%** (Wilson 0++ \(a^2\) dip). Missing conversion \(\sqrt{\sigma} r_0=1+1/(2\pi)\). \(r_0 M\) vs Chen 4.16(11) is 0.82%. | Did not retune \(\varphi^2+1\). Scheme split, not a 6% physics residual. |
| Identifying the scalar glueball with \(f_0(1500)\) or \(f_0(1710)\) | Gluonic \((\varphi^2+1)K=1.520\,\mathrm{GeV}\) is **inside** the PDG T-matrix pole band 1.43–1.53 GeV. BW 1506±6 is the lineshape leftover (0.93%). Flavor \((\pi+1)K\) vs \(f_0(1710)\) is 0.40% green. Morningstar 2025: neither below ~2 GeV is predominantly glue. | Named both PDG \(I=0\) \(0^{++}\) states. Did not retune \(K\). Did not move the BW central. |
| Weather 24 h beating majority-of-saw_storm or ECMWF | Majority used the storm-hold threshold (1010/8) on quiet rows (kill at 1005/12) — **wrong object**. ECMWF RMSE is still a different object. | Storm-sector is the named object; gap-zone should not issue; 44078 is lat-belt transfer to MDXA2; uncoupled clean quiet holds. Frozen issues not rewritten. |
