...
| A SOTA beat as 0.5% green | Riemann n=2..10 locked-\(C\) point-T is still a C-lock timetable vs RvM, not a point-\(T\) residual. Typical \(\lvert S\rvert=\mathrm{POOF}\) ~1% WIP. Signed jitter with prime-3 cancellation is **0.436% green**. Isolated \(\varphi^2+1\) vs Teper is the wrong orifice; σ-coupled is **0.035%**. | Do not stuff the C-lock panel. Do not Euler-invert. Do not put \(\mathrm{POOF}/D\) on \(r_0\). |
| Glueball 6% vs AT2020 \(M/\sqrt{\sigma}\) as an FSOT miss | 1997 vs AT2020 disagree ~7% (Wilson 0++ \(a^2\) dip). σ-unit object is \(\varphi^2+1+\mathrm{POOF}/D_{\mathrm{particle}}\) vs 1997 **0.035%**. Chen vs AT2020-implied \(r_0 M\) is a **5%** scheme split. | Did not retune \(\varphi^2+1\). Did not put flux-tube coupling on \(r_0\). |
| Identifying the scalar glueball with \(f_0(1500)\) or \(f_0(1710)\) | Isolated \((\varphi^2+1)K=1.520\) is inside the T-matrix pole band. BW 1506 is glue–flavor mixing \(V=\mathrm{POOF}\cdot K\) (**0.24%**). Flavor vs 1710 unmixed 0.40%. | Did not retune \(K\). Did not mix 1710. Not a glueball ID. |
