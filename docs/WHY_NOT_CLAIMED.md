...
| Glueball \(m(0^{++})/\sqrt{\sigma}\) vs Teper lattice | Closed mode \(\varphi^2+1\) is 0.876% (0.29σ) — **beats lattice 1σ**, **outside 0.5%**. Do not retune the 3.5 ballpark. | Accuracy WIP. \(\Lambda_{\mathrm{QCD}}\) is a different object. \(\sqrt{2}\) vs 3/2 on \(m(2^{++})/m(0^{++})\) is in 0.5% (aspiration WIP). |
