...
| A SOTA beat as 0.5% green | Riemann n=2..10 locked-\(C\) (1.63%) and glueball \(\varphi^2+1\) vs Teper (0.876%) **beat the competitor** and are **outside** the rest-of-system 0.5% gate. | Marked `beats_sota_fsot_accuracy_wip`. Do not stuff. |
| Glueball \(m(0^{++})/\sqrt{\sigma}\) vs Teper lattice | Closed mode \(\varphi^2+1\) is 0.876% (0.29σ) — **beats lattice 1σ**, **outside 0.5%**. Teper is a **quenched-lattice construct**, not an observed particle. | Accuracy WIP in string units. \(\Lambda_{\mathrm{QCD}}\) is a different object. |
