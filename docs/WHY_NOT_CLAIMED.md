...
| Identifying the scalar glueball with \(f_0(1500)\) or \(f_0(1710)\) | Seed \((\varphi^2+1)K=1.520\,\mathrm{GeV}\) is 0.93% from \(f_0(1500)\) and 12.3% from \(f_0(1710)\). Morningstar 2025: no scalar below ~2 GeV is predominantly glue. Picking the closer is stuffing. | Named **both** PDG \(I=0\) \(0^{++}\) candidates. Did not retune \(K\). |
| Weather 24 h beating majority-of-saw_storm or ECMWF | Majority used the storm-hold threshold (1010/8) on quiet rows (kill at 1005/12) — **wrong object**. ECMWF RMSE is still a different object. | Storm-sector is the named object; quiet-fill stays a miss; thin n_obs<24 is awaiting. Frozen issues not rewritten. |
